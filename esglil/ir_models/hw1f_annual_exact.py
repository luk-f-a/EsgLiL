...
class StochasticDriver:
    ...
    Return an instance of class CorrelatedRV
...
class CashAccount(FunctionOfVariable):
    ...
    Z: random number generator
        ...
        short rate in a specific way, given by the StochasticDriver class
    """ 
...
def h_from_B(B_T, T, t, r, alpha, sigma):
    ...
    h = (-1/alpha*(np.log(B_T)+A*r) +
         sigma**2/2/alpha**3*((T-t) +
...
def b_from_h(t, T, h_T, h_T_1, b, alpha):
    ...
    k_p = (np.exp(alpha)-1)/alpha
    k_m = (1-np.exp(-alpha))/alpha
    next_b = (h_T-h_T_1-k_p*k_m*sum([b[i]*np.exp(-alpha*(T-1-i))
